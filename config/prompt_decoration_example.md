...
**EXPERIMENTAL VARIANT — not the shipped prompt.** Arm B: the same rule PLUS a worked example.

Built 2026-08-20 to test whether a one-shot example steers a VLM where a stated
rule alone does not. It differs from `prompt.md` in exactly two ways: the
"do not skip repeated text" sentence is scoped so it no longer contradicts
`decoration.py`, and the repeated-glyph rule below is added. Arm B adds the example;
Arm A (`prompt_decoration_rule.md`) is otherwise identical and omits it.

Run it with `run_vlm --prompt`. Do not ship it as `prompt.md` without re-running
the whole corpus: prompt and transcripts are a matched pair.
